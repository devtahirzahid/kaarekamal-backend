const MKPMember = require("../database/models/MKPMember");
...
exports.createMember = async (req, res) => {
  try {
    ...
    const newMember = new MKPMember(req.body);
...
exports.getMemberById = async (req, res) => {
  try {
    const member = await MKPMember.findById(req.params.id);
    if (!member) {
...
exports.updateMemberById = async (req, res) => {
  try {
    const updatedMember = await MKPMember.findByIdAndUpdate(
      req.params.id,
      req.body,
...
exports.deleteMemberById = async (req, res) => {
  try {
    const deletedMember = await MKPMember.findByIdAndDelete(req.params.id);
    if (!deletedMember) {
      return res.status(404).json({ message: "Member not found" });
    }
